...
-- COUNT CHECK one by one 
SELECT COUNT(*) FROM users;
SELECT COUNT(*) FROM tutors;

-- COUNT CHECK PER TABLE AS WHOLE 
SELECT 'users' AS tbl, COUNT(*) FROM users
UNION ALL SELECT 'tutors', COUNT(*) FROM tutors
UNION ALL SELECT 'tutor_subjects', COUNT(*) FROM tutor_subjects
...
SELECT COUNT(*) AS null_email FROM users WHERE email IS NULL;
SELECT COUNT(*) AS null_rate  FROM tutors WHERE hourly_rate IS NULL;


-- JOIN CHECK 
SELECT b.id, u.name AS student, t.uid AS tutor
FROM bookings b
JOIN users u ON u.uid = b.student_id
JOIN tutors t ON t.uid = b.tutor_id
LIMIT 10;
